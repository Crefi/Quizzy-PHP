...
$app->get('/questions', function ($request, $response) use ($twig, $view) {
  ...
  $selectedExperince = $queryString['experience_points'];
  $selectedPassing = $queryString['passing_limit'];
...
$app->put('/updateUserExperience', function ($request, $response) use ($controller) {
  $json = $request->getBody();
  $data = json_decode($json, true);

  $email = $data['email'] ?? false;
  $experience_points = $data['experience_points'] ?? false;

  if ($email === false || $experience_points === false) {
    $response->getBody()->write(json_encode('Bad Request!'));
    return $response->withHeader('Content-Type', 'application/json')->withStatus(400);
  }

  // Update the user's experience_points in the database using your desired logic
  // For example, you can use the $controller to update the user's record
  $controller->updateUserExperiencePoints($email, $experience_points);

  $response->getBody()->write(json_encode('Experience Points Updated'));
